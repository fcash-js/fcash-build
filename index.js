 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`fcash-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`fcash-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var buildPath = './node_modules/fcash-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external fcash-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:fcash-lib -o fcash-lib.js';
    }
